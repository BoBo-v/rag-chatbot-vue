 ...
 * 连接阶段按设置超时；收到第一个 chunk 后取消超时计时器，流式阶段不限时
 */
export async function generateStreamWithContext(options: GenerateStreamWithContextOptions): Promise<void> {
    ...
    const timeoutSeconds = normalizeResponseTimeout(runtime.responseTimeoutSeconds)
    const timer = setTimeout(() => timeoutCtrl.abort(), timeoutSeconds * 1000)
    ...
    try {
        ...
        if (timeoutCtrl.signal.aborted && !(signal?.aborted)) {
            throw new DOMException(`Request timed out after ${timeoutSeconds} seconds`, 'TimeoutError')
        }
        throw err
    } finally {
        clearTimeout(timer)
